...
path_Buse = "../Dataset/Buse"
path_Scalabrino = "../Dataset/Scalabrino"
data = pd.read_csv("../Dataset/BuseAndSc.csv")
...
# traverse 360 code snippets in Buse
for i in range(1,101):
    name_ID = data.iat[i - 1, 0]
    index = str(name_ID).replace('Buse', '')
    index = int(index)
    output = subprocess.run(['srcml.exe', path_Buse + "\Buse" + str(index) + ".java"], capture_output=True, check=False)
    ...
    # remove the expression fragment at the beginning of the code （Dorn40）
...
for i in range(1,201):
    name_ID = data.iat[i - 1 + 100, 0]
    index = str(name_ID).replace('Scalabrino', '')
    index = int(index)
    output = subprocess.run(['srcml.exe', path_Scalabrino + "\Scalabrino" + str(index) + ".java"], capture_output=True, check=False)
    root = etree.fromstring(output.stdout)
    ...
    data[i - 1 + 100 : i + 100] = data_fill


# save
data.to_csv("../Result/data_java_all.csv", index=False)